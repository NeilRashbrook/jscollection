...
export class ArrayColl extends KeyValueCollection {
   ...
   */
  remove(item) {
    if (arrayRemove(this._array, item, false)) {
  ...
  async findAsync(filterFunc) {
    for (let [index, item] of this._array.entries()) {
      if (await filterFunc(item, index, this)) {
        return item;
      }
    }
  }
  ...
  removeKey(i) {
    if (i in this._array) {
      this._notifyRemoved(this._array.splice(i, 1));
    }
  }
  ...
  values() {
    return ArrayColl.from(this._array.values());
  }

  keys() {
    return ArrayColl.from(this._array.keys());
  }

  entries() {
    return ArrayColl.from(this._array.entries());
  }
  ...
  async everyAsync(func) {
    for (let [index, item] of this._array.entries()) {
      if (!await func(item, index, this)) {
        return false;
      }
    }
    return true;
  ...
  async everyAsync(func) {
    return !!await this._array.reduce((acc, item, index) => (acc = await acc) && func(acc, item, index, this), true);
  }
  */
  ...
  /* Alternative implementation:
  async someAsync(func) {
    return !!await this._array.reduce((acc, item, index) => (acc = await acc) || func(acc, item, index, this), false);
  }
  */
  ...
  async findIndexAsync(filterFunc) {
    for (let [index, item] of this._array.entries()) {
      if (await filterFunc(item, index, this)) {
        return index;
      }
  ...
  /* Alternative implementation:
  async findIndexAsync(func) {
    return await this._array.reduce((acc, item, index) => (acc = await acc) < 0 && func(acc, item, index, this) ? index : acc, -1);
  }
  */
  ...
  async reduceAsync(func, ...initial) {
    // `acc` is actually a Promise for the result of the previous iteration.
    return await this._array.reduce(async (acc, item, index) => func(await acc, item, index, this), ...initial);
  }
  ...
  async reduceRightAsync(func, ...initial) {
    return await this._array.reduceRight(async (acc, item, index) => func(await acc, item, index, this), ...initial);
  }
  ...
  async flatMapAsync(mapFunc) {
    return new ArrayColl((await Promise.all(this._array.map(mapFunc))).flat());
  }
}
